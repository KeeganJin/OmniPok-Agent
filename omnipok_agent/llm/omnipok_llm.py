...
from ..core.exceptions import OmniPokAgentException
...
class OmniPokLLM:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not all([self.api_key, self.base_url]):
            raise OmniPokAgentException("API密钥和服务地址必须被提供或在.env文件中定义。")
    ...
    def think(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        try:
            ...
            raise OmniPokAgentException(f"LLM调用失败: {str(e)}")
    ...
    def invoke_with_tools(
        ...
        **kwargs
        ...
        Returns:
            包含role, content, tool_calls, finish_reason, usage的字典
        """
        try:
            call_kwargs = {
                "model": self.model,
            ...
            for k, v in kwargs.items():
                if k not in ['temperature', 'max_tokens', 'tool_choice']:
                    call_kwargs[k] = v
